...
1. Determine the platform (the `/moonforge` orchestrator passes it; if absent, detect):
   ...
   - **Unreal** — `*.uproject` present. Its own path, not `generic` — verifying
     it means checking for a Blueprint-callable wrapper and a possible manual
     Blueprint-wiring list, neither of which any other platform has.

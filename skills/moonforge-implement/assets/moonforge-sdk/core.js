...
export function resetAll() {
  state.userProps = {}; state.cacheToken = null; setDistinctId(uuid()); resetSession();
 ...
 *
 * `session_start` fires the moment `init` runs, while a game typically cannot
 * call `identify` until an async session round trip completes. Sending
 * immediately stamps those events with the anonymous id, and `identify` only
 * takes effect going forward - so the earliest events of every session were
 * permanently unattributable to the account that produced them.
 *
